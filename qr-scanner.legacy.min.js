...
console.warn("Note that the return type of scanImage will change in the future. To already switch to the new api today, you can pass returnDetailedScanResult: true.");let t=!!d;try{let m,q;[d,m]=yield Promise.all([d||c.createQrEngine(),c._loadImage(a)]);[f,q]=c._drawToCanvas(m,l,f,g);let n;if(d instanceof Worker){let h=d;t||c._postWorkerMessageSync(h,"inversionMode","both");n=yield new Promise((u,z)=>{let x,v,w,y=-1;v=r=>{r.data.id===y&&(h.removeEventListener("message",v),h.removeEventListener("error",
w),clearTimeout(x),null!==r.data.data?u({stringData:r.data.data,binaryData:r.data.binaryData,version:r.data.version,cornerPoints:c._convertPoints(r.data.cornerPoints,l)}):z(c.NO_QR_CODE_FOUND))};w=r=>{h.removeEventListener("message",v);h.removeEventListener("error",w);clearTimeout(x);z("Scanner error: "+(r?r.message||r:"Unknown Error"))};h.addEventListener("message",v);h.addEventListener("error",w);x=setTimeout(()=>w("timeout"),1E4);let A=q.getImageData(0,0,f.width,f.height);y=c._postWorkerMessageSync(h,
"decode",A,[A.data.buffer])})}else n=yield Promise.race([new Promise((h,u)=>window.setTimeout(()=>u("Scanner error: timeout"),1E4)),(()=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){try{var [h]=yield d.detect(f);if(!h)throw c.NO_QR_CODE_FOUND;return{stringData:h.rawValue,cornerPoints:c._convertPoints(h.cornerPoints,l)}}catch(u){h=u.message||u;if(/not implemented|service unavailable/.test(h))return c._disableBarcodeDetector=!0,c.scanImage(a,{scanRegion:l,canvas:f,disallowCanvasResizing:g,
alsoTryWithoutScanRegion:k});throw`Scanner error: ${h}`;}}))()]);return p?n:n.stringData}catch(m){if(!l||!k)throw m;let q=yield c.scanImage(a,{qrEngine:d,canvas:f,disallowCanvasResizing:g});return p?q:q.stringData}finally{t||c._postWorkerMessage(d,"close")}})}setGrayscaleWeights(a,b,d,f=!0){c._postWorkerMessage(this._qrEnginePromise,"grayscaleWeights",{red:a,green:b,blue:d,useIntegerApproximation:f})}setInversionMode(a){c._postWorkerMessage(this._qrEnginePromise,"inversionMode",a)}static createQrEngine(a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){a&&
...
Math.round(2/3*Math.min(a.videoWidth,a.videoHeight));return{x:Math.round((a.videoWidth-b)/2),y:Math.round((a.videoHeight-b)/2),width:b,height:b,downScaledWidth:this._legacyCanvasSize,downScaledHeight:this._legacyCanvasSize}}_updateOverlay(){requestAnimationFrame(()=>{if(this.$overlay){var a=this.$video,b=a.videoWidth,d=a.videoHeight,f=a.offsetWidth,g=a.offsetHeight,k=a.offsetLeft,l=a.offsetTop,p=window.getComputedStyle(a),t=p.objectFit,m=b/d,q=f/g;switch(t){case "none":var n=b;var h=d;break;case "fill":n=
f;h=g;break;default:("cover"===t?m>q:m<q)?(h=g,n=h*m):(n=f,h=n/m),"scale-down"===t&&(n=Math.min(n,b),h=Math.min(h,d))}var [u,z]=p.objectPosition.split(" ").map((v,w)=>{const y=parseFloat(v);return v.endsWith("%")?(w?g-h:f-n)*y/100:y});p=this._scanRegion.width||b;q=this._scanRegion.height||d;t=this._scanRegion.x||0;var x=this._scanRegion.y||0;m=this.$overlay.style;m.width=`${p/b*n}px`;m.height=`${q/d*h}px`;m.top=`${l+z+x/d*h}px`;d=/scaleX\(-1\)/.test(a.style.transform);m.left=`${k+(d?f-u-n:u)+(d?b-
